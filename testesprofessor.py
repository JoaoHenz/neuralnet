...
if sys.argv[1] =='backpropagation':
    network_struct = gl.read_networkstructfile(sys.argv[2])
    initial_weights = gl.read_initialweightsfile(sys.argv[3])

    #le dataset
    y_column = -1
    data = pd.read_csv(sys.argv[4]) #abre arquivo
...
if sys.argv[1] == 'verificacaogradiente':
    network_struct = gl.read_networkstructfile(sys.argv[2])
    initial_weights = gl.read_initialweightsfile(sys.argv[3])
    ...
    n = NeuralNet(dataset= dataset,initial_weights = initial_weights,y= y, num_entrada = network_struct['num_entrada'],num_saida = network_struct['num_saida'],fator_reg = network_struct['fator_reg'],hidden_lengths =network_struct['hidden_lengths'])
    n.fit(sys.argv[5],verificacaogradiente = True) #arg5 -> numero de iteracoes antes de testar o gradiente
